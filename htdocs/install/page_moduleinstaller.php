...
if (!@require_once dirname(__DIR__) . "/language/{$wizard->language}/global.php") {
    require_once dirname(__DIR__) . '/language/english/global.php';
}
if (!@require_once dirname(__DIR__) . "/modules/system/language/{$wizard->language}/admin/modulesadmin.php") {
    require_once dirname(__DIR__) . '/modules/system/language/english/admin/modulesadmin.php';
}
if (!@require_once dirname(__DIR__) . "/modules/system/language/{$wizard->language}/admin.php") {
    require_once dirname(__DIR__) . '/modules/system/language/english/admin.php';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_once dirname(__DIR__) . '/class/xoopsblock.php';
    require_once dirname(__DIR__) . '/kernel/module.php';
    require_once dirname(__DIR__) . '/include/cp_functions.php';
    require_once dirname(__DIR__) . '/include/version.php';
    ...
    if (count($msgs) > 0) {
        $content = '<div class="alert alert-success"><span class="fa fa-check text-success"></span> ' . INSTALLED_MODULES . '</div><div class="well"><ul class="list-unstyled">';
        foreach ($msgs as $msg) {
    ...
    foreach ($dirlist as $file) {
        ...
        if (!in_array($file, $listed_mods)) {
            ...
            */
            ...
            $content .= '<br><img src="' . XOOPS_URL . '/modules/' . $module->getInfo('dirname') . '/' . $module->getInfo('image') . '" alt="' . $module->getInfo('name') . '">';
            $content .= '</div>';
            $content .= '<div class="col-md-7">';
            $content .= '<h3>' . $module->getInfo('name');
            $content .= ' <small> ' . number_format(round($module->getInfo('version'), 2), 2) . ' (' . $module->getInfo('dirname') . ')' . '</small>' . '</h3>';
