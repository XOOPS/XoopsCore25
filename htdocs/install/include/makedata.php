...
function make_data(&$dbm, $adminname, $hashedAdminPass, $adminmail, $language, $groups)
{
    ...
    // system modules
    if (is_file(dirname(dirname(__DIR__)) . '/modules/system/language/' . $language . '/modinfo.php')) {
        include dirname(dirname(__DIR__)) . '/modules/system/language/' . $language . '/modinfo.php';
    } else {
        include dirname(dirname(__DIR__)) . '/modules/system/language/english/modinfo.php';
        $language = 'english';
    }

    $modversion = array();
    include_once dirname(dirname(__DIR__)) . '/modules/system/xoops_version.php';
    $time = time();
    ...
    foreach ($modversion['templates'] as $tplfile) {
        // Main templates
        if ($fp = fopen(dirname(dirname(__DIR__)) . '/modules/system/templates/' . $tplfile['file'], 'r')) {
            ...
            $tplsource = fread($fp, filesize(dirname(dirname(__DIR__)) . '/modules/system/templates/' . $tplfile['file']));
            fclose($fp);
            $dbm->insert('tplsource', ' (tpl_id, tpl_source) VALUES (' . $newtplid . ", '" . addslashes($tplsource) . "')");
        }
        // Admin templates
        if ($fp = fopen(dirname(dirname(__DIR__)) . '/modules/system/templates/admin/' . $tplfile['file'], 'r')) {
            ...
            // $newtplid = $xoopsDB->getInsertId();
            $tplsource = fread($fp, filesize(dirname(dirname(__DIR__)) . '/modules/system/templates/admin/' . $tplfile['file']));
            fclose($fp);
            $dbm->insert('tplsource', ' (tpl_id, tpl_source) VALUES (' . $newtplid . ", '" . addslashes($tplsource) . "')");
        }
    }
    ...
    foreach ($modversion['blocks'] as $func_num => $newblock) {
        if ($fp = fopen(dirname(dirname(__DIR__)) . '/modules/system/templates/blocks/' . $newblock['template'], 'r')) {
            $visible = 0;
            ...
            // $newtplid = $xoopsDB->getInsertId();
            $tplsource = fread($fp, filesize(dirname(dirname(__DIR__)) . '/modules/system/templates/blocks/' . $newblock['template']));
            fclose($fp);
            $dbm->insert('tplsource', ' (tpl_id, tpl_source) VALUES (' . $newtplid . ", '" . addslashes($tplsource) . "')");
    ...
    $editors = XoopsLists::getDirListAsArray(dirname(dirname(__DIR__)) . '/class/xoopseditor');
    $conf    = 36;
    ...
    $icons = XoopsLists::getDirListAsArray(dirname(dirname(__DIR__)) . '/modules/system/images/icons');
    foreach ($icons as $dir) {
        $dbm->insert('configoption', ' VALUES (' . $conf . ", '" . $dir . "', '" . $dir . "', 98)");
        ++$conf;
    }
    $breadcrumb = XoopsLists::getDirListAsArray(dirname(dirname(__DIR__)) . '/modules/system/images/breadcrumb');
    foreach ($breadcrumb as $dir) {
        $dbm->insert('configoption', ' VALUES (' . $conf . ", '" . $dir . "', '" . $dir . "', 99)");
        ++$conf;
    }
    $jqueryui = XoopsLists::getDirListAsArray(dirname(dirname(__DIR__)) . '/modules/system/css/ui');
