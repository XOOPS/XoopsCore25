...
foreach ($wizard->configs['writable'] as $key => $value) {
    if (is_dir('../' . $value)) {
        $writable .= '<li><span class="far fa-fw fa-folder-open"></span> <strong>' . $value . '</strong></li>';
    } else {
        $writable .= '<li><span class="far fa-fw fa-file-code"></span> <strong>' . $value . '</strong></li>';
    }
}
$writable .= '</ul></div>';
...
foreach ($wizard->configs['xoopsPathDefault'] as $key => $value) {
    $xoops_trust .= '<li><span class="far fa-fw fa-folder-open"></span> <strong>' . $value . '</strong></li>';
}
$xoops_trust .= '</ul></div>';

$writable_trust = '<div class="alert alert-warning"><ul style="list-style: none;">';
foreach ($wizard->configs['dataPath'] as $key => $value) {
    $writable_trust .= '<li><span class="far fa-fw fa-folder-open"></span> <strong>' . $wizard->configs['xoopsPathDefault']['data'] . '/' . $key . '</strong></li>';
    if (is_array($value)) {
        foreach ($value as $key2 => $value2) {
            $writable_trust .= '<li><span class="far fa-fw fa-folder-open"></span> <strong>' . $wizard->configs['xoopsPathDefault']['data'] . '/' . $key . '/' . $value2 . '</strong></li>';
        }
    }
}
