...
if (!function_exists('xoops_module_install_' . $mydirname)) {
    eval('function xoops_module_install_' . $mydirname . '($module) { return protector_oninstall_base($module, ' . var_export($mydirname, true) . '); }');
}
    ...
    function protector_oninstall_base($module, $mydirname)
    {
        ...
        if (!is_dir($protector_data_dir)) {
            if (!mkdir($protector_data_dir, 0755, true) && !is_dir($protector_data_dir)) {
                ...
                trigger_error('Protector install: failed to create data directory', E_USER_WARNING);
                error_log('Protector install: failed to create directory ' . $protector_data_dir);
                return false;
            }
        }
