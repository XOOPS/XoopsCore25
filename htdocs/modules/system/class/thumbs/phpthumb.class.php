...
//		}
	...
	public function RenderToFile($filename) {
		...
		if (($filename[0] != '/') && ($filename[0] != '\\') && ($filename[1] != ':')) { //patched for XOOPS
	...
	public function ResolveFilenameToAbsolute($filename) {
		...
		if (preg_match('#^[a-z0-9]+\:/{1,2}#i', $filename)) {
		...
		} elseif ($this->iswindows && isset($filename[1]) && ($filename[1] == ':')) { //patched for XOOPS

			// absolute pathname (Windows)
			$AbsoluteFilename = $filename;

		} elseif ($this->iswindows && ((substr($filename, 0, 2) == '//') || (substr($filename, 0, 2) == '\\\\'))) { //patched for XOOPS

			// absolute pathname (Windows)
			$AbsoluteFilename = $filename;

		} elseif ($filename[0] == '/') { //patched for XOOPS
			...
			} elseif (isset($filename[1]) && ($filename[1] == '~')) { //patched for XOOPS
	...
	public function ImageMagickCommandlineBase() {
		...
		if (null === $commandline) {
			...
			if ($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)) {
				...
				if ($which_convert && ($which_convert[0] == '/') && $this->file_exists_ignoreopenbasedir($which_convert)) { //patched for XOOPS
