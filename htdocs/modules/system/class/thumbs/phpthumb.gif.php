...
class CGIFLZW
{
	...
	{
		if ($bInit) {
			$this->SetCodeSize = ord($data[0]); //patched for XOOPS
			$data = substr($data, 1);
	...
	public function GetCode(&$data, $bInit)
	{
		...
		if (($this->CurBit + $this->CodeSize) >= $this->LastBit) {
			...
			$Count = ord($data[0]); //patched for XOOPS
			$data  = substr($data, 1);

			if ($Count) {
				for ($i = 0; $i < $Count; $i++) {
					$this->Buf[2 + $i] = ord($data[$i]); //patched for XOOPS
				}
...
class CGIFCOLORTABLE
{
	...
	public function load($lpData, $num)
	{
		...
		for ($i = 0; $i < $num; $i++) {
			...
			$this->m_arColors[] = (ord($rgb[2]) << 16) + (ord($rgb[1]) << 8) + ord($rgb[0]); //patched for XOOPS
			$this->m_nColors++;
		}
...
class CGIFIMAGEHEADER
{
	...
	public function load($lpData, &$hdrLen)
	{
		...
		$b = ord($lpData[8]); //patched for XOOPS
...
class CGIFIMAGE
{
	...
	public function load($data, &$datLen)
	{
		...
		while (true) {
			$b = ord($data[0]); //patched for XOOPS
			$data = substr($data, 1);
			$datLen++;
	...
	public function skipExt(&$data, &$extLen)
	{
		...
		case 0xF9: // Graphic Control
			$b = ord($data[1]); //patched for XOOPS
			$this->m_disp   = ($b & 0x1C) >> 2;
			$this->m_bUser  = ($b & 0x02) ? true : false;
			$this->m_bTrans = ($b & 0x01) ? true : false;
			$this->m_nDelay = $this->w2i(substr($data, 2, 2));
			$this->m_nTrans = ord($data[4]); //patched for XOOPS
			break;

		case 0xFE: // Comment
			$this->m_lpComm = substr($data, 1, ord($data[0])); //patched for XOOPS
			break;
		...
		for ($y = 0; $y < $this->m_gfh->m_nHeight; $y++) {
			for ($x = 0; $x < $this->m_gfh->m_nWidth; $x++, $nPxl++) {
				if (
					...
					// PART OF IMAGE
					if (@$this->m_img->m_bTrans && (ord($data[$nPxl]) == $this->m_img->m_nTrans)) { //patched for XOOPS
						...
						$bmp .= $data[$nPxl]; //patched for XOOPS
					}
				} else {
					// BACKGROUND
					if ($bgColor == -1) {
	...
	public function getGD_PixelPlotterVersion()
	{
		...
		for ($i = 0; $i < $NumColorsInPal; $i++) {
			$ThisImageColor[$i] = imagecolorallocate(
									$PlottingIMG,
									ord($pal[($i * 3) + 0]),  //patched for XOOPS
									ord($pal[($i * 3) + 1]),  //patched for XOOPS
									ord($pal[($i * 3) + 2])); //patched for XOOPS
		}
			...
			for ($x = 0; $x < $this->m_gfh->m_nWidth; $x++, $nPxl++) {
				if (
					...
					if (@$this->m_img->m_bTrans && (ord($data[$nPxl]) == $this->m_img->m_nTrans)) { //patched for XOOPS
						...
						imagesetpixel($PlottingIMG, $x, $this->m_gfh->m_nHeight - $y - 1, $ThisImageColor[ord($data[$nPxl])]); //patched for XOOPS
					}
				} else {
					// BACKGROUND
					imagesetpixel($PlottingIMG, $x, $this->m_gfh->m_nHeight - $y - 1, $ThisImageColor[$this->m_gfh->m_nBgColor]);
