...
/**
 * Bootstrap 5 style form renderer
 *
 * @category  XoopsForm
...
class XoopsFormRendererBootstrap5 implements XoopsFormRendererInterface
{
	...
	protected function renderCheckedInline($element, $type, $elementId, $elementName)
	{
		...
		foreach ($elementOptions as $value => $name) {
			...
			$ret .= '<div class="form-check form-check-inline me-3">';
	...
	{
		$count = 0;
		$ret = '<span class="d-inline-flex align-items-center">';
	...
	{
		return '<div class="form-control-plaintext">' . $element->getValue() . '</div>';
	}
	...
	public function renderFormTextDateSelect(XoopsFormTextDateSelect $element)
	{
		...
		return '<div class="input-group">'
			...
			. '<span class="input-group-text"><button class="btn btn-secondary" type="button"'
	...
	public function renderThemeForm(XoopsThemeForm $form)
	{
		...
		foreach ($form->getElements() as $element) {
			...
			if (($caption = $element->getCaption()) != '') {
				$ret .= '<label for="' . $element->getName() . '" class="col-md-3 col-form-label text-end">'
					. $element->getCaption()
					. ($element->isRequired() ? '<span class="text-danger">*</span>' : '')
					. '</label>';
			} else {
		...
		$ret .= $hidden;
		if ($form->getRequired()) {
			$ret .= '<div class="col-12 mb-2"> <span class="text-danger">*</span> = Required</div>';
		}
		$ret .= '</form></div>';
	...
	public function addThemeFormBreak(XoopsThemeForm $form, $extra, $class)
	{
		...
		$form->addElement('<div class="col-12 ' . $class . '">' . $extra . '</div>');
	}
}
