...
if ($isadmin || ($catreadcount > 0) || ($catwritecount > 0)) {
    ...
    if (!empty($_GET['op']) && $op === 'delcat') {
        ...
        xoops_confirm(['op' => 'delcatok', 'imgcat_id' => $imgcat_id, 'target' => $target], 'xoopsimagebrowser.php', _MD_RUDELIMGCAT);
        xoops_footer();
        exit();
    }
    ...
    if (!empty($_GET['op']) && $op === 'delfile') {
        xoops_header();
        echo "<link href='css/xoopsimagebrowser.css' rel='stylesheet' type='text/css' />";
        xoops_confirm(['op' => 'delfileok', 'image_id' => $image_id, 'target' => $target], 'xoopsimagebrowser.php', _MD_RUDELIMG);
        xoops_footer();
        exit();
    }
