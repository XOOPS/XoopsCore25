...
class XoopsFormDhtmlTextArea extends XoopsFormTextArea
{
    ...
    public function render()
    {
        ...
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            $ret .= "<button type='button' class='btn btn-default btn-sm' onclick=\"XoopsCheckLength('" . $this->getName() . "', '" . @$this->configs['maxlength'] . "', '" . _XOOPS_FORM_ALT_LENGTH . "', '" . _XOOPS_FORM_ALT_LENGTH_MAX . "');\" title='" . _XOOPS_FORM_ALT_CHECKLENGTH . "'><span class='glyphicon glyphicon-question-sign' aria-hidden='true'></span></button>&nbsp;";
            $ret .= "</div></div>";
        } else {
        ...
        if (empty($this->skipPreview)) {
            ...
            if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
                $button = "<button type='button' class='btn btn-primary' onclick=\"form_instantPreview('" . XOOPS_URL . "', '" . $this->getName() . "','" . XOOPS_URL . "/images', " . (int)$this->doHtml . ", '" . $GLOBALS['xoopsSecurity']->createToken() . "')\" title='" . _PREVIEW . "'>" . _PREVIEW . "</button>";
            } else {
                $button = '<input ' . "   id='" . $this->getName() . "_preview_button'" . "   type='button' " . "   value='" . _PREVIEW . "' " . "   onclick=\"form_instantPreview('" . XOOPS_URL . "', '" . $this->getName() . "','" . XOOPS_URL . "/images', " . (int)$this->doHtml . ", '" . $GLOBALS['xoopsSecurity']->createToken() . "')\"" . ' />';
            }
    ...
    public function codeIcon()
    {
        ...
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            ...
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='xoopsCodeUrl(\"{$textarea_id}\", \"" . htmlspecialchars(_ENTERURL, ENT_QUOTES) . "\", \"" . htmlspecialchars(_ENTERWEBTITLE, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_URL . "'><span class='glyphicon glyphicon-link' aria-hidden='true'></span></button>&nbsp;";
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='xoopsCodeEmail(\"{$textarea_id}\", \"" . htmlspecialchars(_ENTEREMAIL, ENT_QUOTES) . "\", \"" . htmlspecialchars(_ENTERWEBTITLE, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_EMAIL . "'><span class='glyphicon glyphicon-envelope' aria-hidden='true'></span></button>&nbsp;";
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='xoopsCodeImg(\"{$textarea_id}\", \"" . htmlspecialchars(_ENTERIMGURL, ENT_QUOTES) . "\", \"" . htmlspecialchars(_ENTERIMGPOS, ENT_QUOTES) . "\", \"" . htmlspecialchars(_IMGPOSRORL, ENT_QUOTES) . "\", \"" . htmlspecialchars(_ERRORIMGPOS, ENT_QUOTES) . "\", \"" . htmlspecialchars(_XOOPS_FORM_ALT_ENTERWIDTH, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_IMG . "'><span class='glyphicon glyphicon-picture' aria-hidden='true'></span></button>&nbsp;";
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='openWithSelfMain(\"" . XOOPS_URL . "/imagemanager.php?target={$textarea_id}\",\"imgmanager\",400,430);' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_IMAGE . "'><span class='glyphicon glyphicon-picture' aria-hidden='true'></span> Manager</button>&nbsp;";
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='openWithSelfMain(\"" . XOOPS_URL . "/misc.php?action=showpopups&amp;type=smilies&amp;target={$textarea_id}\",\"smilies\",300,475);' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_SMILEY . "'><span class='glyphicon glyphicon-sunglasses' aria-hidden='true'></span> " . _XOOPS_FORM_ALT_SMILEY . "</button>&nbsp;";
        } else {
        ...
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='xoopsCodeCode(\"{$textarea_id}\", \"" . htmlspecialchars(_ENTERCODE, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_CODE . "'><span class='glyphicon glyphicon-menu-left' aria-hidden='true'></span><span class='glyphicon glyphicon-menu-right' aria-hidden='true'></span> " . _XOOPS_FORM_ALT_CODE . "</button>&nbsp;";
            $code .= "<button type='button' class='btn btn-default btn-xs' onclick='xoopsCodeQuote(\"{$textarea_id}\", \"" . htmlspecialchars(_ENTERQUOTE, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"hand\"' title='" . _XOOPS_FORM_ALT_QUOTE . "'><span class='glyphicon glyphicon-bullhorn' aria-hidden='true'></span> " . _XOOPS_FORM_ALT_QUOTE . "</button>&nbsp;";
            $code .= "</div></div>";
        } else {
    ...
    public function fontArray()
    {
        ...
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            $fontStr = "<script type=\"text/javascript\">" . "var _editor_dialog = ''" . "+ '<div class=\'row\'><div class=\'col-md-4\'><select class=\"input-sm form-control\" id=\'{$textarea_id}Size\' onchange=\'xoopsSetElementAttribute(\"size\", this.options[this.selectedIndex].value, \"{$textarea_id}\", \"{$hiddentext}\");\'>'" . "+ '<option value=\'SIZE\'>" . _SIZE . "</option>'";
        } else {
        ...
        $fontStr .= " + '</select> '";
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            $fontStr .= " + '</div><div class=\'col-md-4\'> '";
        }            
        ...
        if ($GLOBALS['xoopsConfig']['bootstrap'] == true){
            ...
            $styleStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeBold(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_BOLD . "' aria-label='Left Align'><span class='glyphicon glyphicon-bold' aria-hidden='true'></span></button>";
            $styleStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeItalic(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_ITALIC . "' aria-label='Left Align'><span class='glyphicon glyphicon-italic' aria-hidden='true'></span></button>";
            $styleStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeUnderline(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_UNDERLINE . "' aria-label='Left Align'>" . _XOOPS_FORM_ALT_UNDERLINE . "</button>";
            $styleStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeLineThrough(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_LINETHROUGH . "' aria-label='Left Align'>" . _XOOPS_FORM_ALT_LINETHROUGH . "</button>";
            $styleStr .= "</div>";
            
            $alignStr = "<div class='btn-group' role='group'>";
            $alignStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeLeft(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_LEFT . "' aria-label='Left Align'><span class='glyphicon glyphicon-align-left' aria-hidden='true'></span></button>";
            $alignStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeCenter(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_CENTER . "' aria-label='Left Align'><span class='glyphicon glyphicon-align-center' aria-hidden='true'></span></button>";
            $alignStr .= "<button type='button' class='btn btn-default btn-sm' onclick='xoopsMakeRight(\"{$hiddentext}\", \"{$textarea_id}\");' title='" . _XOOPS_FORM_ALT_RIGHT . "' aria-label='Left Align'><span class='glyphicon glyphicon-align-right' aria-hidden='true'></span></button>";
