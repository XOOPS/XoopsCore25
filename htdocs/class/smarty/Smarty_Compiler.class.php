...
/* $Id: Smarty_Compiler.class.php 12921 2014-12-26 17:48:08Z beckmi $ */
...
class Smarty_Compiler extends Smarty
{
    ...
    public function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        if ('' !== ($compiled_content) && (substr($compiled_content, -1) === "\n")) {
     ...
     * @param  string $val
     * @return string
     * @internal param string $tag_attrs
     */
    ...
    public function _compile_plugin_call($type, $name)
    {
        if (isset($this->_plugins[$type][$name])) {
            ...
            if (is_array($this->_plugins[$type][$name][0])) {
                return ((is_object($this->_plugins[$type][$name][0][0])) ? "\$this->_plugins['$type']['$name'][0][0]->"    /* method callback */ : (string)($this->_plugins[$type][$name][0][0]) . '::'    /* class callback */) . $this->_plugins[$type][$name][0][1];
            } else {
                /* function callback */
