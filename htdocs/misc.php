...
//                return;
            ...
            if ($smiles = $myts->getSmileys()) {
                ...
                foreach ($smiles as $key => $smile) {
                    echo "<tr class='$rcolor'><td>" . $smile['code'] . '</td><td>' . $smile['emotion'] . "</td><td><img onmouseover='style.cursor=\"hand\"' onclick='doSmilie(\" " . $smile['code'] . " \");' src='" . XOOPS_UPLOAD_URL . '/'
                         . $smile['smile_url'] . "' alt='' /></td></tr>";
        ...
        <table width='100%'>
            <tr>
                ...
                foreach ($avatarslist as $file => $name) {
                    echo '<td><img src="uploads/' . $file . '" alt="' . $name . '" style="padding:10px; vertical-align:top;"  /><br>' . $name . '<br><input name="myimage" type="button" value="' . _SELECT . '" onclick="myimage_onclick(' . $counter
                         . ')" /></td>';
                    ++$counter;
                    ++$cntavs;
                    if ($cntavs > 8) {
                ...
                case 'online':
                    ...
                    for ($i = 0; $i < $count; ++$i) {
                        ...
                        if (is_object($onlineUsers[$i]['user'])) {
                            ...
                            echo '<td>' . $avatar . "</td><td><a href=\"javascript:window.opener.location='" . XOOPS_URL . '/userinfo.php?uid=' . $onlineUsers[$i]['user']->getVar('uid') . "';window.close();\">"
                    ...
                    break;
                case 'ssllogin':
                    if ($xoopsConfig['use_ssl'] && isset($_POST[$xoopsConfig['sslpost_name']])
                        && is_object($xoopsUser)
                    ) {
