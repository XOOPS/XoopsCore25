    ...
    <div class="col-md-6">
        <{if !$user_ownpage && $xoops_isuser == true}>
            <form name="usernav" action="user.php" method="post">
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$smarty.const._PROFILE_MA_SENDPM}>"
        ...
        <{if isset($user_ownpage) && $user_ownpage == true}>
            <form name="usernav" action="user.php" method="post">
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_editprofile}>"
                       onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname}>/edituser.php'">
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_changepassword}>"
                       onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname}>/changepass.php'">
                <{if isset($user_changeemail)}>
                    <input class="btn btn-primary btn-sm w-100" type="button" value="<{$smarty.const._PROFILE_MA_CHANGEMAIL}>"
                           onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname}>/changemail.php'">
                <{/if}>
                <{if isset($user_candelete) && $user_candelete == true}>
                    <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_deleteaccount}>" onclick="location='user.php?op=delete'">
                <{/if}>
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_avatar}>" onclick="location='edituser.php?op=avatarform'">
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_inbox}>" onclick="location='<{$xoops_url}>/viewpmsg.php'">
                <input class="btn btn-primary btn-sm w-100" type="button" value="<{$lang_logout}>"
        ...
        <{elseif $xoops_isadmin != false}>
            <form method="post" action="<{$xoops_url}>/modules/<{$xoops_dirname}>/admin/deactivate.php">
                <input class="btn btn-warning btn-sm w-100" type="button" value="<{$lang_editprofile}>"
                ...
                <{if isset($userlevel) && $userlevel == 1}>
                    ...
                    <input class="btn btn-danger btn-sm w-100" type="button" value="<{$smarty.const._PROFILE_MA_DEACTIVATE}>" onclick="submit();">
                <{else}>
                    <input type="hidden" name="level" value="1">
                    <input class="btn btn-warning btn-sm w-100" type="button" value="<{$smarty.const._PROFILE_MA_ACTIVATE}>" onclick="submit();">
                <{/if}>
            </form>
        <{/if}>
    </div><!-- .col-md-6 -->
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item list-group-item-action">
                            ...
                            <{if $path_image_overloaded|file_exists}>
                                ...
                                <div class="d-inline text-muted"><small><span class="fa-solid fa-calendar fa-sm ms-2"></span> <{$result.time}></small></div>
                                <br />
                            <{else}>
                                <img src="<{$result.image}>" alt="<{$module.name}>"> <a href="<{$result.link}>"><{$result.title}></a> <small><span class="fa-solid fa-calendar fa-sm ms-2"></span> <{$result.time}></small><br />
                            <{/if}>
                        </li>
                        <{/foreach}>
                    </ul>
