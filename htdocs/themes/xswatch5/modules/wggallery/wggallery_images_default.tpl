...
<div class='card panel-<{$panel_type|default:'none'}> mb-3'>
	<{if isset($showlist)}>
                ...
                <{foreach item=image from=$images|default:null}>
                    <div id='imglist_<{$image.id}>' class='row wgg-img-panel wgg-image-list'>
                        ...
                        <div class='wgg-img-panel-row col-sm-12 center'>
                            <{if isset($showModal)}>
                                <a href='' id='btnModal<{$image.id}>' class='btn btn-secondary wgg-btn' data-bs-toggle='modal' data-bs-target='#myModalImagePicker<{$image.id}>'>
        ...
        <div class='clear'>&nbsp;</div>
            <{if isset($pagenav)}>
                <div class="col">
                <div class="generic-pagination xo-pagenav float-end mb-2"><{$pagenav|replace:'form':'div'|replace:'id="xo-pagenav"':''|replace:' //':'/'}></div>
                </div>
            <{/if}>
        </div>
    <{/if}>
