...
class XoopsGroupHandler extends XoopsObjectHandler
{
    ...
    public function insert(XoopsObject $group)
    {
        ...
        if ($group->isNew()) {
            ...
            $sql     = sprintf('INSERT INTO %s (groupid, name, description, group_type) VALUES (%u, %s, %s, %s)', $this->db->prefix('groups'), $groupid, $this->db->quoteString($name), $this->db->quoteString($description),
                               $this->db->quoteString($group_type));
        } else {
            $sql = sprintf('UPDATE %s SET name = %s, description = %s, group_type = %s WHERE groupid = %u', $this->db->prefix('groups'), $this->db->quoteString($name), $this->db->quoteString($description), $this->db->quoteString($group_type),
                           $groupid);
        }
        if (!$result = $this->db->query($sql)) {
            return false;
